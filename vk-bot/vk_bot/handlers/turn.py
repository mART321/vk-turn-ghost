...
_INSTRUCTION = (
    "🔑 Обход белых списков\n\n"
    "Работает даже при ограничениях — когда стандартный VPN не подключается.\n\n"
    "Как подключиться:\n"
    "1. Скачай приложение GhostMode — инструкция в разделе «📖 Как пользоваться»\n"
    "2. Добавь подписку по ссылке из «📡 Мой VPN → Ключ»\n"
    "3. В приложении выбери конфиг «Обход белых списков»\n"
    "4. Нажми «Подключить»\n\n"
    "Если не работает — нажми кнопку ниже и получи код активации."
)
...
async def _has_active_tunnel(vk_id: int) -> bool:
    ...
    async with managed_session() as db:
        ...
        result2 = await db.execute(
            select(Tunnel).where(Tunnel.user_id == user.id, Tunnel.active == True).limit(1)
        )
        return result2.scalars().first() is not None
...
async def cmd_turn_get_link(message: Message):
    ...
    await message.answer(
        "📋 Код активации:\n\n"
        f"{encrypted}\n\n"
        "Скопируй код → открой GhostMode → Настройки → Обход белых списков → вставь → Применить.",
        keyboard=turn_menu_kb(),
    )
