...
HAPP_WINDOWS  = "https://github.com/Happ-proxy/happ-desktop/releases/latest/download/setup-Happ.x64.exe"
HAPP_MACOS    = "https://github.com/Happ-proxy/happ-desktop/releases/latest/download/Happ.macOS.universal.dmg"
...
def tariff_kb(device: str) -> str:
    kb = Keyboard(one_time=False, inline=False)
    kb.add(Text("⚡ Fast — 250 ₽/мес", payload={"cmd": "tunnel_tier", "device": device, "tier": "standard"}))
    kb.row()
    kb.add(Text("🧅 Ghost — 500 ₽/мес", payload={"cmd": "tunnel_tier", "device": device, "tier": "tor"}))
    kb.row()
...
def howto_kb() -> str:
    kb = Keyboard(one_time=False, inline=False)
    kb.add(OpenLink(HAPP_ANDROID, "🤖 Android — Happ"))
    kb.row()
    kb.add(Text("🍎 iPhone — выбрать приложение", payload={"cmd": "howto_iphone"}))
    kb.row()
    kb.add(OpenLink(HAPP_WINDOWS, "🪟 Windows — Happ"))
    kb.row()
...
def howto_iphone_kb() -> str:
    kb = Keyboard(one_time=False, inline=False)
    kb.add(OpenLink(HAPP_IOS, "⭐ Happ (рекомендуем)"))
    kb.row()
...
def howto_macos_kb() -> str:
    kb = Keyboard(one_time=False, inline=False)
    kb.add(OpenLink(HAPP_MACOS, "⬇️ Скачать Happ для macOS"))
    kb.row()
    kb.add(Text("⬅️ Назад", payload={"cmd": "howto"}))
    return kb.get_json()
